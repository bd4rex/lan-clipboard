...
### 2026-09-15 15:30:14 CST 通用入口发布 / Universal Picker Release

- 仓库与提交 / Repository and commit: `https://github.com/bd4rex/lan-clipboard`，功能提交 / feature commit `049c13513b0ee8fe9df393b614b9ec2df3c9b7ff` 已快进合并并推送到 `main`，远端引用已核实。Fast-forwarded and pushed to main, with the remote ref verified.
- 发布结果 / Release result: 已备份旧代码、配置和 SQLite 后热更新，13 个发布文件校验一致；服务未重启，PID 和原内存文件数量未变，HTTP 健康检查及 SQLite 完整性检查通过。Backed up code, configuration, and SQLite before hot update; all 13 release-file hashes match. No restart, unchanged PID and existing memory-file count, with passing HTTP health and SQLite integrity checks.
- 验证与配置 / Verification and configuration: 本地 32 项 Python + 10 项 Node.js、服务器暂存版本 32 项 Python 测试通过。原生标签点击可打开浏览器文件选择器，上传下载与三种视口布局通过；HTTP、端口 `8765`、10 GiB 上限及 1800 秒默认保留不变，后端和配置文件校验未变。Local 32 Python and 10 Node.js tests and 32 server staging tests passed. Native label activation, upload/download, and three viewport layouts passed; protocol, port, limits, retention, backend, and configuration remain unchanged.
- 记录同步 / Record sync: 本条发布记录随后作为独立文档提交推送并同步服务器，不回填自身提交号；不包含运行数据、部署凭据或真实主机信息。This release record follows as a separate documentation commit and server sync without embedding its own commit ID, runtime data, credentials, or real host details.
